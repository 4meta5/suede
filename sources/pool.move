...
module suede::pool {
    ...
    /// Entry point for the `swap_token_b` method. Sends swapped tokenA
    /// to the sender.
    entry fun swap_token_b_<P, A, B>(
        pool: &mut Pool<P, A, B>, token_b: Coin<B>, ctx: &mut TxContext
    ) {
        transfer::public_transfer(
            swap_token_b(pool, token_b, ctx),
            tx_context::sender(ctx)
        )
    }
    ...
    /// `Coin<A>` and `Coin<B>`, and in exchange he gets `Coin<LSP>` -
    ...
    public fun add_liquidity<P, A, B>(
        ...
        let token_amt = balance::join(&mut pool.token_a, token_a_balance);
        let token_b_amt = balance::join(&mut pool.token_b, token_balance);

        assert!(token_amt < MAX_POOL_VALUE && token_b_amt < MAX_POOL_VALUE, EPoolFull);
    ...
    public fun remove_liquidity<P, A, B>(
        ...
        let (token_amt, token_b_amt, lsp_supply) = get_amounts(pool);
        let token_a_removed = (token_amt * lsp_amount) / lsp_supply;
    ...
    /// - How much tokenA one will get if they send `to_sell` amount of T;
...
module suede::pool_tests {
    ...
    #[test] fun test_swap_token_a() {
        let scenario = scenario();
        test_swap_token_a_(&mut scenario);
        test::end(scenario);
    }
    #[test] fun test_swap_token_b() {
        let scenario = scenario();
    ...
    fun test_init_pool_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            let pool = test::take_shared<Pool<POOLEY, SUI, BEEP>>(test);
    ...
    fun test_swap_token_a_(test: &mut Scenario) {
        ...
        next_tx(test, the_guy);
        {
            ...
            let token = pool::swap_token_a(pool_mut, mint<SUI>(5000000, ctx(test)), ctx(test));
    ...
    fun test_swap_token_b_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            ...
            let sui = pool::swap_token_b(pool_mut, mint<BEEP>(1000, ctx(test)), ctx(test));
    ...
    fun test_withdraw_almost_all_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            let lsp = mint<LSP<POOLEY, SUI, BEEP>>(31622000 - 1, ctx(test));
            let pool = test::take_shared<Pool<POOLEY, SUI, BEEP>>(test);
            let pool_mut = &mut pool;
    ...
    fun test_withdraw_all_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            let lsp = mint<LSP<POOLEY, SUI, BEEP>>(31622000, ctx(test));
            let pool = test::take_shared<Pool<POOLEY, SUI, BEEP>>(test);
            let pool_mut = &mut pool;
